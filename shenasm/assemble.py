...
def assemble_instruction(issues: IssueLog, chip: ChipInfo, symbols: typing.Dict[str, Symbol], labels: typing.List, inst: [Instruction]):
    ...
    :param labels: the label table from the label_pass
    ...
    for given_arg, expected_type in zip_longest(inst.args, info.argtypes):
        ...
        if given_arg in symbols:
            if inst.mneumonic != 'jmp':
                args.append(symbols[given_arg].value)
            else:
                issues.error(
                    inst.source_pos,
                    '{} instruction has non-label argument "{}"',
                    inst.mneumonic,
                    given_arg
                )
            ...
            if inst.mneumonic == 'jmp':
                ...
                issues.error(
                    inst.source_pos,
                    '{} instruction has label argument "{}"',
                    inst.mneumonic,
                    given_arg
                )
...
def label_pass(instructions: [Instruction]) -> typing.List:
    ...
    for inst in instructions:
        if inst.label:
            result.append(inst.label[:-1]) # strip the trailing :
    return result